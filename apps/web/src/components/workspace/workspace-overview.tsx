...
import { getDomainLabel, getRecentContextsForProfile, getRoleLabel, withProfile } from "@/lib/demo-data";
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 pb-6">
      <ScopeHeader
        ...
        role={activeProfile ? getRoleLabel(activeProfile.role) : "Loading"}
        course={self?.courseLabel ?? activeProfile?.courseLabel ?? "context loading"}
        classNameLabel={self?.classLabel ?? activeProfile?.classLabel ?? "context loading"}
        domain={getDomainLabel(self?.domain ?? activeProfile?.domain ?? "academic")}
      />
        ...
        <div className="panel-card flex min-h-[520px] items-center justify-center px-6 py-8">
          <div className="max-w-2xl rounded-[24px] border border-dashed border-[var(--danger)] bg-[var(--danger-soft)]/50 px-6 py-7">
            ...
            <p className="mt-3 text-sm leading-7 text-[var(--body)]">
              profile registry나 canonical context를 읽는 단계에서 문제가 생겼습니다. backend를 다시 확인한 뒤 context bootstrap을 재시도하면 role-aware shell이 실제 응답 기준으로 다시 정렬됩니다.
            </p>
            <button
              type="button"
          ...
          <section className="panel-card px-6 py-6 lg:px-7">
            <div className="max-w-3xl space-y-2">
              ...
              <p className="text-sm leading-7 text-[var(--body)]">
                이제 이 화면은 실제 backend context profile registry를 기준으로 동작합니다. 학생은 질문과 학습 기록을, 강사는 반복 질문과 공식 지식 반영을, 검토자는 승격 품질과 maintenance를 중심으로 같은 시스템을 다른 관점에서 보게 됩니다.
              </p>
            </div>
                ...
                <div className="mt-4 space-y-3">
                  {recentContexts.map((context) => (
                    <Link
                      ...
                      href={withProfile(context.href, activeProfile?.profileId ?? context.profileId)}
            ...
            <section className="panel-card px-5 py-5">
              ...
              <div className="mt-4 space-y-3 text-sm leading-6 text-[var(--body)]">
                <div className="rounded-[20px] border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4">
                  ...
                  <p className="mt-1">{activeProfile ? `${getRoleLabel(activeProfile.role)} · ${self?.actorId ?? activeProfile.actorId}` : "loading"}</p>
                </div>
                <div className="rounded-[20px] border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4">
                  <p className="font-semibold text-[var(--foreground)]">Course / Class</p>
                  <p className="mt-1">{self?.courseLabel ?? activeProfile?.courseLabel} · {self?.classLabel ?? activeProfile?.classLabel}</p>
                </div>
                <div className="rounded-[20px] border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4">
                  <p className="font-semibold text-[var(--foreground)]">Domain</p>
                  <p className="mt-1">{getDomainLabel(self?.domain ?? activeProfile?.domain ?? "academic")}</p>
                </div>
              </div>
            </section>
          </div>
