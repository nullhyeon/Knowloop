...
function Invoke-CodexCriticPackages {
    ...
    foreach ($package in $packages) {
        ...
        if ($packageCanNarrow) {
            Write-Host "Codex Critic timed out on a wide package. Narrowing scope and retrying..."
            $nextMaxDiffLines = [Math]::Max(120, [int][Math]::Floor($CurrentMaxDiffLines / 2))
            if ($nextMaxDiffLines -ge $CurrentMaxDiffLines) {
                throw "Codex Critic cannot narrow package '$($package.Path)' any further after timing out."
            }
